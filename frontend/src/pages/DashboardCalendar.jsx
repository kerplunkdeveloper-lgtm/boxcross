...
import { 
  Calendar as CalendarIcon, ChevronLeft, ChevronRight, Plus, MapPin, DollarSign, 
  ExternalLink, Save, X, Upload, Loader2, RefreshCw, Clock, AlignLeft, Info, Trash2, Edit 
} from "lucide-react";
import { 
  getEventsListAdmin, createEventItem, updateEventItem, deleteEventItem, getBookings 
} from "../api/api";
...
const DashboardCalendar = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(error);
      toast.error(error.response?.data?.message || "An error occurred while saving the event", { id: toastId });
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="text-left">
            ...
            <p className="text-[var(--db-text-muted)] text-xs md:text-sm mt-1">
              Visual monthly calendar view. Click any day to create events. Manage bookings and class schedules in real-time.
            </p>
          </div>
        ...
        <div className="bg-[var(--db-card)] border border-[var(--db-card-border)] rounded-[24px] overflow-hidden shadow-2xl p-6 md:p-8 transition-colors">
          ...
          <div className="flex items-center justify-between mb-8 pb-4 border-b border-[var(--db-card-border)]">
            <div className="flex items-center gap-2">
              <CalendarIcon className="text-[var(--db-accent-highlight)]" size={20} />
              <h2 className="text-lg font-black uppercase tracking-widest text-[var(--db-text-title)]" style={{ fontFamily: '"Brutal Font", sans-serif' }}>
          ...
          {loading ? (
            <div className="py-32 flex flex-col items-center justify-center text-[var(--db-text-muted)] text-xs gap-2">
              ...
              <p className="uppercase tracking-widest font-black text-[10px]">Loading schedules...</p>
            </div>
          ) : (
                ...
                <div className="grid grid-cols-7 divide-x divide-y divide-[var(--db-card-border)] bg-[var(--db-card-border)]/20">
                  {calendarCells.map((cell, idx) => {
                    ...
                    const cellEvents = events.filter(evt => 
                      evt.schedules && evt.schedules.some(sch => {
                    ...
                    return (
                      <div 
                        key={idx} 
                        onClick={() => user?.role === "admin" && handleCellClick(cell.date)}
        ...
        {showFormModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            ...
            >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-sm font-black uppercase tracking-wider text-[var(--db-text-title)]">
                      {editMode ? "Modify Scheduled Event" : "Create New Event Schedule"}
                    </h3>
                    <p className="text-[10px] text-[var(--db-text-muted)]">Configure details, time slots, and cover banner image</p>
                  </div>
                </div>
                <button
                  onClick={() => setShowFormModal(false)}
              ...
              {/* Form container */}
              <form onSubmit={handleSubmit} className="flex-grow overflow-y-auto p-6 space-y-6 custom-scrollbar text-left">
                  ...
                  >
                    {imagePreview ? (
                      ...
                      <div className="text-center space-y-2">
                        ...
                        <div className="text-xs font-bold text-[var(--db-text)]">Click or Drop to Upload Image</div>
                        <div className="text-[10px] text-[var(--db-text-muted)]">PNG, JPG, WEBP (Ratio 16:9 or 21:9)</div>
                      </div>
                    )}
                    <input 
                      type="file"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="space-y-1">
                    ...
                    <div className="relative">
                      <DollarSign size={14} className="absolute left-3.5 top-3.5 text-[var(--db-text-muted)]" />
                      <input
                        type="number"
                        value={price}
                ...
                <div className="space-y-4 pt-4 border-t border-[var(--db-card-border)]">
                  <div className="flex items-center justify-between">
                    <h4 className="text-[10px] font-black uppercase tracking-wider text-[var(--db-text-muted)] flex items-center gap-1.5">
                      <CalendarIcon size={12} className="text-[var(--db-accent-highlight)]" />
                    ...
                    <button
                      ...
                      onClick={() => {
                        setSchedules([
                          ...schedules,
                          { date: formatDateToCustom(new Date()), timeSlots: [{ time: "9:00 AM", slots: 20, booked: 0 }] }
                        ]);
                      }}
                  ...
                  {schedules.map((schedule, sIndex) => (
                    <div 
                      key={sIndex}
                      className="p-4 bg-[var(--db-input-bg)] border border-[var(--db-input-border)] rounded-2xl relative space-y-4"
                    >
                      {/* Date details row */}
                      <div className="flex items-end justify-between gap-4">
                        <div className="flex-grow space-y-1">
                          ...
                          <div className="relative">
                            <CalendarIcon size={12} className="absolute left-3 top-2.5 text-[var(--db-text-muted)]" />
                            <input
                              type="text"
                              value={schedule.date}
                              onChange={(e) => {
                        ...
                        <button
                          type="button"
                          onClick={() => setSchedules(schedules.filter((_, idx) => idx !== sIndex))}
                      ...
                      <div className="pl-3 border-l border-[var(--db-card-border)] space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          <button
                            ...
                            onClick={() => {
                              const updated = [...schedules];
                              updated[sIndex].timeSlots.push({ time: "10:00 AM", slots: 20, booked: 0 });
                              setSchedules(updated);
                            }}
                        ...
                        {schedule.timeSlots.map((slot, tIndex) => (
                          <div key={tIndex} className="flex items-center gap-2 bg-[var(--db-bg)] p-2 rounded-xl border border-[var(--db-card-border)]">
                            <input
                              ...
                              placeholder="9:00 AM"
                              className="w-full bg-transparent border-none outline-none text-xs text-[var(--db-text)]"
                              required
                            />
                            <div className="w-24 flex items-center gap-1.5 bg-white/5 px-2 py-1 rounded-lg">
                              <span className="text-[8px] text-[var(--db-text-muted)] font-bold uppercase">Limit:</span>
                              <input
                                type="number"
                                value={slot.slots}
                                onChange={(e) => {
                            ...
                            <button
                              ...
                              onClick={() => {
                                const updated = [...schedules];
                                updated[sIndex].timeSlots.splice(tIndex, 1);
                                setSchedules(updated);
                              }}
                              className="p-1 hover:bg-red-500/10 text-red-400 hover:text-red-300 rounded"
                            >
                              <X size={12} />
                            </button>
                          </div>
