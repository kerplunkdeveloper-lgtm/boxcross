...
const PhonePeModal = ({ isOpen, onClose, planDetails }) => {
  ...
  const validate = () => {
    ...
    if (!/^[6-9]\d{9}$/.test(user.phone)) e.phone = "Enter a valid 10-digit number";
  ...
  const features = ["Unlimited group classes", "Open gym access", "BXC community access", "Progress tracking"];
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[99999] flex items-end md:items-center justify-center md:p-6 font-sans">
          ...
          <motion.div
            ...
            animate={{ opacity: 1, y: 0, scale: 1, transition: { type: "spring", damping: 26, stiffness: 300 } }}
          ...
          >
            ...
            <div className="hidden md:flex flex-col justify-between w-[340px] shrink-0 bg-[#0f0f13] text-white p-8 relative overflow-hidden">
              ...
              <div className="absolute -bottom-20 -right-10 w-56 h-56 bg-[#defb02]/10 rounded-full blur-3xl pointer-events-none" />

              <div className="relative z-10">
                {/* Logo */}
                <div className="flex items-center gap-3 mb-8">
                  <div className="w-10 h-10 bg-[#5f259f] rounded-2xl flex items-center justify-center shadow-lg">
                    <span className="text-white font-black text-lg leading-none pt-0.5">पे</span>
                  </div>
                  <div>
                    <p className="font-black text-sm tracking-wide">PhonePe</p>
                    <p className="text-white/50 text-[10px] uppercase tracking-widest">Secure Checkout</p>
                  </div>
                </div>

                {/* Step Indicator */}
                <div className="space-y-4 mb-8">
                  {[{ n: 1, label: "Your Details", sub: "Name, phone & email" }, { n: 2, label: "Payment", sub: "UPI / QR Code" }].map(s => (
                    <div key={s.n} className="flex items-center gap-4">
                      <div className={`w-9 h-9 rounded-full flex items-center justify-center font-black text-sm border-2 shrink-0 transition-all duration-300 ${
                        step > s.n ? "bg-[#defb02] border-[#defb02] text-black" :
                        step === s.n ? "bg-[#5f259f] border-[#5f259f] text-white" :
                        "bg-transparent border-white/20 text-white/30"
                      }`}>
                        {step > s.n ? <Check size={16} /> : s.n}
                      </div>
                      <div>
                        <p className={`font-bold text-sm ${step >= s.n ? "text-white" : "text-white/30"}`}>{s.label}</p>
                        <p className={`text-[11px] ${step >= s.n ? "text-white/50" : "text-white/20"}`}>{s.sub}</p>
                      </div>
                    </div>
                  ))}
                  <div className="ml-4 w-px h-8 bg-white/10 -mt-2" style={{ marginLeft: "17px" }} />
                </div>

                {/* Plan Card */}
                {planDetails && (
                  <div className="bg-white/5 border border-white/10 rounded-2xl p-4 backdrop-blur-sm">
                    <p className="text-white/50 text-[10px] uppercase tracking-widest font-bold mb-1">Selected Plan</p>
                    <p className="text-white font-bold text-sm line-clamp-2 mb-3">{planDetails.name}</p>
                    <div className="flex items-end gap-1">
                      <span className="text-[#defb02] font-black text-3xl">₹{planDetails.price}</span>
                      <span className="text-white/40 text-[11px] mb-1">one-time</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="relative z-10 mt-6">
                <p className="text-white/40 text-[10px] uppercase tracking-widest font-bold mb-3">What's included</p>
                <div className="space-y-2">
                  {features.map((f, i) => (
                    <div key={i} className="flex items-center gap-2 text-[10px] text-white/60">
                      <div className="w-4 h-4 rounded-full bg-[#defb02]/10 border border-[#defb02]/30 flex items-center justify-center shrink-0">
                        <Check size={9} className="text-[#defb02]" />
                      </div>
                      {f}
                    </div>
                  ))}
                ...
                <div className="flex items-center gap-2 mt-5 text-[11px] text-white/30 font-bold">
                  <ShieldCheck size={13} className="text-green-400" /> 256-bit encrypted & secure
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col min-h-0">
              ...
              <div className="md:hidden bg-gradient-to-r from-[#5f259f] to-[#8b3fcf] px-5 py-4 flex items-center justify-between text-white shrink-0">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-white rounded-full flex items-center justify-center">
                    <span className="text-[#5f259f] font-black text-base pt-0.5">पे</span>
                  </div>
                  <div>
                    <p className="font-black text-sm">PhonePe</p>
                    <p className="text-white/70 text-[10px] uppercase tracking-widest">Step {step} of 2</p>
                  </div>
                </div>
                {!busy && <button onClick={onClose} className="p-1.5 hover:bg-white/20 rounded-full"><X size={18} /></button>}
              </div>
              ...
              {!busy && (
                <div className="hidden md:flex items-center justify-between px-8 pt-7 pb-2 shrink-0">
                  <div>
                    <p className="text-[#5f259f] text-[11px] font-bold uppercase tracking-widest mb-0.5">Step {step} of 2</p>
                    <h2 className="text-gray-900 font-black text-xl">{step === 1 ? "Your Details" : "Choose Payment"}</h2>
                  </div>
              ...
              {!busy && (
                <div className="md:hidden bg-gray-50 border-b border-gray-100 px-5 py-3 flex items-center justify-between shrink-0">
                  <p className="text-gray-600 text-[12px] font-semibold line-clamp-1 flex-1 mr-3">{planDetails?.name}</p>
                  <p className="text-gray-900 font-black text-lg shrink-0">₹{planDetails?.price}</p>
                </div>
              )}
                    ...
                    >
                      ...
                      <div>
                        ...
                        <input type="text" value={user.name}
                          onChange={e => { setUser(p => ({ ...p, name: e.target.value })); setErrors(p => ({ ...p, name: "" })); }}
                        ...
                        />
                        {errors.name && <p className="text-red-500 text-[11px] mt-1.5 flex items-center gap-1"><Info size={10} />{errors.name}</p>}
                      </div>
                      ...
                      <div>
                        ...
                        <div className="relative">
                          <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 font-bold text-sm border-r border-gray-200 pr-3">+91</span>
                          <input type="tel" value={user.phone} maxLength={10}
                            onChange={e => { setUser(p => ({ ...p, phone: e.target.value.replace(/\D/g, "") })); setErrors(p => ({ ...p, phone: "" })); }}
                        ...
                        {errors.phone && <p className="text-red-500 text-[11px] mt-1.5 flex items-center gap-1"><Info size={10} />{errors.phone}</p>}
                      </div>
                      ...
                      <div>
                        ...
                        <input type="email" value={user.email}
                          onChange={e => { setUser(p => ({ ...p, email: e.target.value })); setErrors(p => ({ ...p, email: "" })); }}
                        ...
                        />
                        {errors.email && <p className="text-red-500 text-[11px] mt-1.5 flex items-center gap-1"><Info size={10} />{errors.email}</p>}
                      </div>
                  ...
                  {step === 2 && payState === "idle" && (
                    ...
                    >
                      ...
                      <div className="flex-1 px-5 md:px-8 py-5 md:py-6 flex flex-col md:border-r border-gray-100">
                        ...
                        >
                          <div className="w-9 h-9 rounded-full bg-[#5f259f] flex items-center justify-center text-white font-black group-hover:scale-110 transition-transform shadow-md">पे</div>
                          <span className="text-sm font-bold text-[#5f259f]">Open PhonePe App</span>
                        </button>

                        <div className="flex items-center gap-3 mb-5">
                          <div className="h-px bg-gray-200 flex-1" />
                          <span className="text-[9px] text-gray-400 font-bold uppercase tracking-widest">OR ENTER UPI ID</span>
                          <div className="h-px bg-gray-200 flex-1" />
                        </div>
                        ...
                        />
                        {upiError && <p className="text-red-500 text-[11px] mt-1.5 flex items-center gap-1"><Info size={10} />{upiError}</p>}
                        ...
                        <button onClick={() => setStep(1)} className="mt-4 text-[11px] text-gray-400 hover:text-[#5f259f] font-semibold flex items-center gap-1 transition-colors">
                      ...
                      <div className="flex-1 bg-gray-50/50 px-5 md:px-8 py-5 md:py-6 flex flex-col items-center justify-center border-t md:border-t-0 border-gray-100">
                        ...
                        <div className="bg-white p-4 border-2 border-gray-200 rounded-3xl shadow-xl relative group cursor-pointer hover:border-[#5f259f]/40 transition-colors">
                          {[
                            ...
                            <div key={i} className={`absolute w-5 h-5 border-[#5f259f] ${cls} transition-transform duration-300`} />
                          ))}
                        ...
                        <p className="text-gray-400 text-[11px] mt-4 mb-4 text-center leading-relaxed">
                          Scan with any UPI app to pay <span className="font-black text-gray-800">₹{planDetails?.price}</span>
                        </p>
                    ...
                    >
                      <div className="relative mb-8">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                          <Smartphone size={28} className="text-[#5f259f] animate-pulse" />
                        </div>
                      </div>
                      <h3 className="text-gray-900 font-black text-2xl mb-2">Awaiting Approval</h3>
                      <p className="text-gray-400 text-sm mb-8 max-w-xs leading-relaxed">
                        {upiId ? `Approve the request in your UPI app for ${upiId}` : "Complete the payment in your UPI app"}
                      </p>
                      <div className="w-full max-w-xs bg-gray-100 rounded-full h-1.5 overflow-hidden">
                        <motion.div initial={{ width: "0%" }} animate={{ width: "100%" }} transition={{ duration: 4.5, ease: "linear" }}
                          className="h-full bg-gradient-to-r from-[#5f259f] to-[#8b3fcf] rounded-full" />
                      </div>
                      <p className="text-[11px] text-gray-400 font-bold uppercase tracking-widest mt-5">Do not close this window</p>
                    </motion.div>
                  )}

                  {/* ── SUCCESS ── */}
                  {payState === "success" && (
                    <motion.div key="succ" initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                    ...
                    >
                      ...
                      >
                        ...
                        <CheckCircle2 size={54} className="text-green-500 z-10" />
                      </motion.div>
                      <h3 className="text-gray-900 font-black text-2xl md:text-3xl mb-1">Payment Successful!</h3>
                      <p className="text-gray-400 text-sm mb-7">₹{planDetails?.price} paid to Boxcross Fitness</p>
                      <div className="bg-gray-50 w-full max-w-sm p-5 rounded-2xl border border-gray-200 text-left shadow-sm space-y-3 text-[12px]">
                        {[["Name", user.name], ["Phone", `+91 ${user.phone}`], ["Email", user.email], ["Plan", planDetails?.name]].map(([k, v]) => (
                          <div key={k} className="flex justify-between items-center">
                            <span className="text-gray-400 font-bold uppercase tracking-wider">{k}</span>
                            <span className="font-bold text-gray-900 truncate ml-4 max-w-[180px]">{v}</span>
                          </div>
                        ))}
                        <div className="flex justify-between items-center pt-3 border-t border-dashed border-gray-200">
                          <span className="text-gray-400 font-bold uppercase tracking-wider">Txn ID</span>
                          <span className="font-black text-[#5f259f]">TXN{Math.floor(Math.random() * 1e10)}</span>
                        </div>
                      </div>
                      <div className="mt-7 flex flex-col items-center">
                        <Loader2 size={20} className="text-[#5f259f] animate-spin mb-2" />
                        <p className="text-[11px] text-gray-400 font-bold uppercase tracking-widest">Redirecting...</p>
                      </div>
                    </motion.div>
                  )}
              ...
              {!busy && (
                <div className="border-t border-gray-100 py-3 flex items-center justify-center gap-2 text-[10px] text-gray-400 font-bold tracking-wide shrink-0 bg-gray-50/50">
                  <ShieldCheck size={12} className="text-green-500" /> 100% SECURE PAYMENTS BY PHONEPE
                </div>
              )}
            </div>
          </motion.div>
