...
const EventCalender = ({ events = [] }) => {
  ...
  useEffect(() => {
    const daysWithEvents = Object.keys(eventMap)
      .map(Number)
      .sort((a, b) => a - b);
  ...
  const weekday = selectedFullDate.toLocaleDateString("en-GB", {
    weekday: "short",
  });
  const month = selectedFullDate.toLocaleDateString("en-GB", {
    month: "short",
  });
        ...
        <div className="w-full lg:w-[50%] bg-[#0a0a0a] rounded-[2rem] p-6 md:p-10 border border-white/5 shadow-2xl relative overflow-hidden h-fit">
          ...
          <div className="flex items-center justify-between mb-8 relative z-10">
            <button
              ...
              className="p-2.5 bg-white/5 hover:bg-white/10 rounded-full text-white transition-all border border-white/5 hover:border-white/10 group cursor-pointer"
            >
              <ChevronLeft
                size={18}
          ...
          <div className="grid grid-cols-7 gap-y-4 gap-x-2 text-center relative z-10">
            {days.map((day) => (
              <div
                key={day}
                className="text-[10px] md:text-[11px] font-black uppercase tracking-widest text-gray-500 mb-2"
              >
                {day}
              </div>
            ))}
              ...
              return (
                <div key={date} className="flex justify-center">
                  ...
                  >
                    ...
                    {dotColor && (
                      <span
                        className={`absolute bottom-1.5 md:bottom-2 w-1 h-1 rounded-full ${dotColor} shadow-[0_0_8px_currentColor]`}
                      />
                    )}
                  </button>
                </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-8 relative z-10">
                      <h3
                        className="text-xl md:text-2xl font-black text-white leading-tight sm:pr-4"
                        style={{ fontFamily: '"BrutalTypeBold", sans-serif' }}
                      >
                        {event.title}
                      </h3>
                    ...
                    <div className="space-y-5 relative z-10 mb-8">
                      {timeSlot && (
                        <div className="flex items-start gap-4 text-gray-300 group/item">
                          ...
                          <div className="flex flex-col mt-0.5">
                            <span className="text-[12px] font-bold text-gray-500 uppercase tracking-wider mb-0.5">
                              Time
                            </span>
                      ...
                      <div className="flex items-start gap-4 text-gray-300 group/item">
                        ...
                        <div className="flex flex-col mt-0.5">
                          ...
                          <span className="text-lg font-semibold tracking-wide text-gray-200 leading-relaxed">
                            {event.location}
                          </span>
                        </div>
                      </div>
                      ...
                      {timeSlot && (
                        <div className="flex items-start gap-4 text-gray-300 group/item">
                          ...
                          <div className="flex flex-col mt-0.5">
                            ...
                            <span className="text-lg font-semibold tracking-wide text-gray-200">
                              {timeSlot.slots - (timeSlot.booked || 0)} spots
                              available
                            </span>
                          </div>
                        </div>
