...
const EventList = () => {
  ...
  const handleShareToWhatsApp = () => {
    if (!bookingEvent || !selectedSlot) return;

    const dateStr = bookingEvent.schedules?.[activeDateIndex]?.date || "";
    const timeStr = selectedSlot.time || "";
    const locationStr = bookingEvent.location || "Box & Cross Arena";

    const textMessage = `🔥 *REGISTRATION CONFIRMED!* 🔥
Gym: *Box & Cross*
...
• *Reserved Seats:* ${seatsCount} Seat${seatsCount > 1 ? "s" : ""}
...
Thank you for registering! We've reserved your spot and look forward to seeing you. 💪🏋️`;

    const encodedText = encodeURIComponent(textMessage);
    const whatsappUrl = `https://api.whatsapp.com/send?phone=918925556900&text=${encodedText}`;
        ...
        {loading ? (
          ...
          >
            {displayEvents.map((event) => {
              // Calculate slot availability stats
              let totalSlots = 0;
              let totalBooked = 0;
              let hasSlots = false;
                ...
                event.schedules.forEach((schedule) => {
                  if (schedule.timeSlots && Array.isArray(schedule.timeSlots)) {
                    schedule.timeSlots.forEach((slot) => {
                      const cap = Number(slot.slots) || 0;
                      const bkd = Number(slot.booked) || 0;
                      totalSlots += cap;
                      totalBooked += bkd;
                      hasSlots = true;
                    });
              ...
              const remaining = Math.max(0, totalSlots - totalBooked);
              const isSoldOut = hasSlots && totalSlots > 0 && remaining === 0;
              const isFillingFast =
                hasSlots &&
                totalSlots > 0 &&
                remaining > 0 &&
